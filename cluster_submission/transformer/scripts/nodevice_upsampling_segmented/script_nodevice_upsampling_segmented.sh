...
for num_encoderlayers in "${num_encoderlayers_values[@]}"; do
    ...
    echo "#SBATCH --job-name=upseg" >> "$script_name"
    ...
    echo "time srun python /home/sharareh.sayyad/HAR/deeplearning_activity_recognition/Transformer/transformer_wisdm_nodevice_withupsampling_segmented.py $path $max_length_timestep $modelname $embed_size $nhead $dim_feedforward $num_encoderlayers $conv1d_kernel_size $size_linear_layers $conv1d_emb $dropout $batch_size $patience $norm_type $lr $segmented_duration  $upsampling $PWD/" >> "$script_name"
    chmod +x "$script_name"
