...
output_file="transformer_nodevice_upsampling.txt"
outputsorted_file="transformer_nodevice_upsampling_sorted.txt"

# Initialize the output file with headers
echo "batch_size transformermodel upsampling lr max_length_series nhead_encoder conv1d_kernel_size bool_conv1d_emb embed_size dim_feedforward norm_type num_encoderlayers size_linear_layers accuracy f1_score gmean precision recall" > "$output_file"

# Find all .out files in folders starting with "har_"
find nodevice_segmented/nodevice_* -type f -name "*.out" | while read -r file; do
...
norm_type=$(extract_value "norm_type")
    ...
    echo "$batch_size\t$transformermodel\t$upsample\t$lr\t$max_length_series\t$nhead_encoder\t$conv1d_kernel_size\t$bool_conv1d_emb\t$embed_size\t$dim_feedforward\t$norm_type\t$num_encoderlayers\t$size_linear_layers\t$accuracy\t$f1_score\t$gmean\t$precision\t$recall" >> "$output_file"
