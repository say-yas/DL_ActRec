...
for size_linear_lyr in "${size_linear_lyr_values[@]}"; do
    ...
    for initial_channels in "${initial_channels_values[@]}"; do
        ...
        echo "time srun python /home/sharareh.sayyad/HAR/deeplearning_activity_recognition/CNN_LSTM/cnn_wisdm_nodevice_withdownupsampling_segmented.py $path $max_length_timestep $num_conv_lyrs $size_linear_lyr $num_blocks_per_layer $initial_channels $lstm_hidden_size $lstm_layer $batch_size $patience $modelname $norm_type $lr $segmented_duration  $doupsampling $PWD/" >> "$script_name"
