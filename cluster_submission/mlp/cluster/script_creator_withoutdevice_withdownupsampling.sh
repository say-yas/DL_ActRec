...
num_hidden_lyr_values=(8 16) # 8 16 32)
hidden_lyr_size_values=(10 20 26 32) # 16 32)
...
for num_hidden_lyr in "${num_hidden_lyr_values[@]}"; do
    for hidden_lyr_size in "${hidden_lyr_size_values[@]}"; do
        ...
        dir_name="har_nodevice_${num_hidden_lyr}_${hidden_lyr_size}_${modelname}_batch${batch_size}_p${patience}_lr${lr}_${doupsampling}"
        mkdir -p "$dir_name"
        ...
        echo "#SBATCH --cpus-per-task=20" >> "$script_name"
        ...
        echo "" >> "$script_name"
        echo "time srun python /home/sharareh.sayyad/HAR/deeplearning_activity_recognition/MLP/mlp_wisdm_nodevice_withdownupsampling.py $path $num_hidden_lyr $hidden_lyr_size $batch_size $patience $modelname $lr $doupsampling $PWD/" >> "$script_name"
