...
num_hidden_lyr_values=(8 ) # 8 16 32)
hidden_lyr_size_values=(36 42) # 16 32)
...
for num_hidden_lyr in "${num_hidden_lyr_values[@]}"; do
    for hidden_lyr_size in "${hidden_lyr_size_values[@]}"; do
        ...
        dir_name="har_nodevice_${num_hidden_lyr}_${hidden_lyr_size}_${modelname}_batch${batch_size}_p${patience}_lr${lr}_${upsampling}"
        mkdir -p "$dir_name"
        ...
        echo "" >> "$script_name"
        echo "time srun python /home/sharareh.sayyad/HAR/deeplearning_activity_recognition/MLP/mlp_wisdm_nodevice_withupsampling.py $path $num_hidden_lyr $hidden_lyr_size $batch_size $patience $modelname $lr $upsampling $PWD/" >> "$script_name"
