...
num_conv_lyrs_values=(2) # 8 16 32)
size_linear_lyr_values=(8) # 10) # 16 32)
hidden_lyr_size_values=(16) # 16 32)
num_blocks_per_layer_values=(1)
...
lrs=(1e-4 1e-5)
max_length_timesteps=(500 1000 4000)
...
for size_linear_lyr in "${size_linear_lyr_values[@]}"; do
    ...
    for initial_channels in "${initial_channels_values[@]}"; do
        ...
        dir_name="nodevice_${modelname}_batch${batch_size}_p${patience}_norm${norm_type}_maxt${max_length_timestep}_conv${num_conv_lyrs}_lnr${size_linear_lyr}_lstm${lstm_layers}_lstmsize${lstm_hidden_size}_hdlyrsz${hidden_lyr_size}_nblocks${num_blocks_per_layer}_inich${initial_channels}_lr${lr}"
        mkdir -p "$dir_name"
        ...
        echo "time srun python /home/sharareh.sayyad/HAR/deeplearning_activity_recognition/CNN_LSTM/cnn_wisdm_nodevice.py $path $max_length_timestep $num_conv_lyrs $size_linear_lyr $num_blocks_per_layer $initial_channels $lstm_hidden_size $lstm_layers $batch_size $patience $modelname $norm_type $lr $PWD/" >> "$script_name"
