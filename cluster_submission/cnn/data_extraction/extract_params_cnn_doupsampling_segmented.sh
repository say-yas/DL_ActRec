...
echo "batch_size model_type doupsample lr max_length_series segment_duration num_conv_lyrs size_linear_lyr num_blocks_per_layer initial_channels lstm_hidden_size lstm_layers norm_type accuracy f1_score gmean precision recall" > "$output_file"

# Find all .out files in folders starting with "har_"
find nodevice_doupsampling_segmented/nodevice_* -type f -name "*.out" | while read -r file; do
