...
output_file="transformer_nodevice_doupsampling_segmented.txt"
outputsorted_file="transformer_nodevice_doupsampling_segmented_sorted.txt"

# Initialize the output file with headers
echo "batch_size transformermodel doupsample lr max_length_series segment_duration nhead_encoder conv1d_kernel_size bool_conv1d_emb embed_size dim_feedforward norm_type num_encoderlayers size_linear_layers mean_accuracy mean_f1_score mean_gmean mean_precision mean_recall std_accuracy std_f1_score std_gmean std_precision std_recall total_params" > "$output_file"
