...
norm_type=$(extract_value "norm_type")
    ...
    echo "$batch_size\t$model_type\t$doupsample\t$lr\t$max_length_series\t$segment_duration\t$num_conv_lyrs\t$size_linear_lyr\t$num_blocks_per_layer\t$initial_channels\t$lstm_hidden_size\t$lstm_layers\t$norm_type\t$mean_acc\t$mean_f1\t$mean_gmean\t$mean_prec\t$mean_rec\t$std_acc\t$std_f1\t$std_gmean\t$std_prec\t$std_rec\t$total_params" >> "$output_file"
