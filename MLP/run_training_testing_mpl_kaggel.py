...
import nn_mlp_model as mlp
import nn_mlp_train as mlp_train
...
def create_increasing_decreasing_list(n, initial_size, final_size):
  ...
  increasing_part = list([ 1*i for i in range(initial_size, n // 2 + initial_size)])
  decreasing_part = list([ 2*i for i in range(final_size + (n // 2), final_size, -1)])
...
def run_mlp_training(data, labels, classes, device, n_features=6, n_classes=2, test_size=0.3,
                      ...
                      lr=0.001, num_epochs=100,  patience=5, modeltype = "mlp1",
                        num_hidden_lyr=2, hidden_lyr_size=100, opt="adamW", verbose=False):
    ...
    parameters = {"test_size": test_size, "val_size": val_size, "batch_size": batch_size, "num_cpus": num_cpus,
                "lr": lr, "num_epochs": num_epochs, "verbose": verbose, "n_features": n_features, "n_classes": n_classes,
                    "patience": patience, "num_hidden_lyr": num_hidden_lyr, "hidden_lyr_size": hidden_lyr_size}
    ...
    summary(model.to(device), input_size=(batch_size, n_features)) 
    ...
    optimizer=optim.Adam(model.parameters(),lr=lr) if opt=="adam" else optim.AdamW(model.parameters(),lr=lr) 
    loss_function=nn.CrossEntropyLoss()
    early_stopper = mlp.EarlyStopper(verbose=verbose, path="checkpoint.pt", patience=patience)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min',
                                                                    factor=0.9, patience=patience-2,
                                                                        threshold=1e-8)
    ...
    # train model
    train_losses, val_losses, train_accs, val_accs, confusion_matrix = train_model.run_training(trainloader, valloader)
    ...
    mlp_train.plot_progress(strcomplexity+"loss","loss", train_losses, val_losses, save_path="./loss.png")
    mlp_train.plot_progress(strcomplexity+"Accuracy","Accuracy", train_accs, val_accs, save_path="./accuracy.png")

    # test model
    correct, total, accuracy_test = train_model.testing_step(testloader)
    cm = train_model.get_confusion_matrix(testloader, view_cm=False)
    mlp_train.visualize_confusion_matrix(cm, classes, correct, total, path="./confusion_matrix_testloader.png")
    ...
    if verbose:
        ...
        sn.heatmap(df_cm/ (df_cm.astype("float").sum(axis=1) + 1e-9), annot=True)
        ax.set_title(strcomplexity)
        ax.set_xlabel("Predicted")
        ax.set_ylabel("True")
        plt.show()
        ax.figure.savefig("./confusion_matrix_test.png")
