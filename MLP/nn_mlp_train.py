...
class TrainMLP:
    ...
    def generate_train_val_test(self, x, y):
        ...
        print("counts classes in train:", y_train.value_counts(), "ib validation:", y_val.value_counts(), "in test:", y_test.value_counts())

        
        
        scaler = StandardScaler()
        x_train_scaled = scaler.fit_transform(x_train)  # Standardization of training set
        x_val_scaled = scaler.transform(x_val)  # Use training mean & std for validation set
        x_test_scaled = scaler.transform(x_test) # Use training mean & std for test set
    ...
    def training_step(self, dataloader, master_bar):
        ...
        for x, y in fastprogress.progress_bar(dataloader, parent=master_bar):
            ...
            # compute f1score
            f1score = multiclass_f1_score(y_pred.argmax(dim=1), y.to(self.device), num_classes=self.params["n_classes"], average="macro")
            epoch_f1score.append(f1score.cpu().numpy())
        ...
        return np.mean(epoch_loss), accuracy(epoch_correct, epoch_total), np.mean(epoch_f1score)
    ...
    def validatation_step(self, dataloader, master_bar):
        ...
        with torch.no_grad():
            for x, y in fastprogress.progress_bar(dataloader, parent=master_bar):
                ...
                # compute f1score
                f1score = multiclass_f1_score(y_pred.argmax(dim=1), y.to(self.device),
                                               num_classes=self.params["n_classes"],
                                                 average="macro")
                epoch_f1score.append(f1score.cpu().numpy())
        ...
        return np.mean(epoch_loss), accuracy(epoch_correct, epoch_total), confusion_matrix, np.mean(epoch_f1score)
    ...
    def run_training(self, train_dataloader, val_dataloader):
        ...
        train_losses, val_losses, train_accs, val_accs, train_f1, val_f1 = [],[],[],[],[],[]

        for epoch in master_bar:
            # Train the model
            epoch_train_loss, epoch_train_acc, epoch_train_f1 = self.training_step(train_dataloader, master_bar)
            # Validate the model
            epoch_val_loss, epoch_val_acc, confusion_matrix, epoch_val_f1 = self.validatation_step(val_dataloader, master_bar)
            ...
            if verbose:
                print('Train loss:', epoch_train_loss, 'val loss:', epoch_val_loss, 'train acc:', epoch_train_acc, 'val acc:', epoch_val_acc)
                # master_bar.write(f'Train loss: {epoch_train_loss:.2f}, val loss: {epoch_val_loss:.2f},train acc: {epoch_train_acc:.3f}, val acc: {epoch_val_acc:.3f}')

            # Early stopping to prevent overfitting
            if (epoch>10 and self.early_stopper):
                self.early_stopper.update(epoch_val_loss, self.model)
        ...
        return train_losses, val_losses, train_accs, val_accs, confusion_matrix, train_f1, val_f1

    def get_confusion_matrix(self, test_loader, view_cm=False):
        """Compute confusion matrix on test set.
        Args:
            test_loader (DataLoader): Torch DataLoader object to load the
                test data
            view_cm (bool, optional): If True, print confusion matrix. Defaults
                to False.
        Returns:
    ...
    def testing_step(self, test_loader):
        ...
        return correct, total, accuracy(correct, total), float(f1score)
    
    
def data_to_tensor(data, labels):

    torch_lbls = torch.tensor(labels.values)
    torch_data = torch.tensor(data.values.astype(np.float32)) # this type is needed for mps devices as float64 is not supported 
