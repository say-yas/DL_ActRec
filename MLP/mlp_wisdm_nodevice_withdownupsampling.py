...
try:
    ...
    if len(sys.argv) < 7:
        print("add path to data (with slash at the end of path), number of hidden layers, hidden layer size, batch size, patience, model, lr, downupsample and pathsave")
    else:
        pathdata = sys.argv[1]
        ...
        mlpmodel = sys.argv[6]
        lr = float(sys.argv[7])
        doupsample = sys.argv[8]
        pathsave = sys.argv[9]
    print("num_hidden_lyr, hidden_lyr_size, batch_size, patience, mlpmodel, lr, downupsample:", num_hidden_lyr, hidden_lyr_size, batch_size, patience, mlpmodel, lr, doupsample)
    ...
    x, y = doupsampling.fit_resample(x, y)
