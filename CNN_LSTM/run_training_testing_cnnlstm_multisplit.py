...
def run_cnnlstm_training(data, labels, classes, device, num_channels=6, n_classes=2, test_size=0.3,
    ...
    initial_channels: int
        number of initial channels in the network
    lstm_hidden_size: int
        number of hidden units in the LSTM network
    num_lstm_layers: int
        number of LSTM layers in the network
    num_training: int
        number of times training samples
    verbose: bool
    ...
    for idx in range(num_training):
        ...
        early_stopper = cnn.EarlyStopper(verbose=verbose, path="checkpoint_idx"+str(idx+1)+".pt", patience=patience)
        ...
        cnn_train.plot_progress(strcomplexity+"loss","loss", train_losses, val_losses, save_path=pathsave+"loss_idx"+str(idx+1)+".png")
        cnn_train.plot_progress(strcomplexity+"Accuracy","Accuracy", train_accs, val_accs, save_path=pathsave+"accuracy_idx"+str(idx+1)+".png")
        cnn_train.plot_progress(strcomplexity+"F1score","F1score", train_f1, val_f1, save_path=pathsave+"f1score_idx"+str(idx+1)+".png")
        cnn_train.plot_progress(strcomplexity+"Gmean","Gmean", train_gmean, val_gmean, save_path=pathsave+"gmean_idx"+str(idx+1)+".png")
        cnn_train.plot_progress(strcomplexity+"Precision","Precision", train_pre, val_pre, save_path=pathsave+"precision_idx"+str(idx+1)+".png")
        cnn_train.plot_progress(strcomplexity+"Recall","Recall", train_rec, val_rec, save_path=pathsave+"recall_idx"+str(idx+1)+".png")
        ...
        cnn_train.visualize_confusion_matrix(cm, classes, correct, total, path=pathsave+"confusion_matrix_testloader_idx"+str(idx+1)+".png")
