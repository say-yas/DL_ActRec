...
try:
    ...
    if len(sys.argv) < 14:
        print("path_data/, max_length_series, num_conv_lyrs, size_linear_lyr, num_blocks_per_layer, initial_channels, lstm_hidden_size, num_lstm_layers, batch_size, patience, cnnlstmmodel, norm_type, lr")
    else:
        pathdata = sys.argv[1]
        ...
        doupsample = sys.argv[14]
        pathsave = sys.argv[15]
        if len(sys.argv) >= 16:
            num_training = int(sys.argv[16])
        else:
            num_training = 1
