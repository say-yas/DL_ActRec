...
def run_transformer_training(data, labels, classes, device, num_channels=6, n_classes=2, test_size=0.3,
                      ...
                      lr=0.001, num_epochs=100,  patience=5, modeltype = "trans1",
                        ...
                        opt="adamW", verbose=False, pathsave="./", weights=None, norm_type="per-channel", num_training=20):
    ...
    num_training: int
        number of training runs to perform. Defaults to 1.
    Returns:
    ...
    for idx in range(num_training):
        print(f"Training run {idx+1}/{num_training}")
        ...
        early_stopper = transformer.EarlyStopper(verbose=verbose, path="checkpoint_idx"+str(idx+1)+".pt", patience=patience)
        ...
        if modeltype =="trans1":
            strcomplexity = f"idx={idx+1}: TransformerI, [{num_input, num_output, max_length_series, nhead, dim_feedforward, num_encoderlayers, conv1d_emb, conv1d_kernel_size, size_linear_layers }]: "


        transformer_train.plot_progress(strcomplexity+"loss","loss", train_losses, val_losses, save_path=pathsave+"loss_idx"+str(idx+1)+".png")
        transformer_train.plot_progress(strcomplexity+"Accuracy","Accuracy", train_accs, val_accs, save_path=pathsave+"accuracy_idx"+str(idx+1)+".png")
        transformer_train.plot_progress(strcomplexity+"F1score","F1score", train_f1, val_f1, save_path=pathsave+"f1score_idx"+str(idx+1)+".png")
        transformer_train.plot_progress(strcomplexity+"Gmean","Gmean", train_gmean, val_gmean, save_path=pathsave+"gmean_idx"+str(idx+1)+".png")
        transformer_train.plot_progress(strcomplexity+"Precision","Precision", train_pre, val_pre, save_path=pathsave+"precision_idx"+str(idx+1)+".png")
        transformer_train.plot_progress(strcomplexity+"Recall","Recall", train_rec, val_rec, save_path=pathsave+"recall_idx"+str(idx+1)+".png")
        ...
        transformer_train.visualize_confusion_matrix(cm, classes, correct, total, path=pathsave+"confusion_matrix_testloader_idx"+str(idx+1)+".png")
        print(f"training idx: {idx+1}")
        print(f"Accuracy on test set at idx={idx+1} : {accuracy_test:.4f}")
        print(f"F1score on test set at idx={idx+1} : {f1score_test:.4f}")
        print(f"Gmean on test set at idx={idx+1} : {gmean_test:.4f}")
        print(f"Precision on test set at idx={idx+1} : {precision_test:.4f}")
        print(f"Recall on test set at idx={idx+1} : {recall_test:.4f}")
        ...
        if verbose:
            ...
            ax.set_ylabel("True")
            # plt.show()
            ax.figure.savefig(pathsave+"confusion_matrix_test_allnormalized_idx"+str(idx+1)+".png")
    ...
    print(f"arr_acc_test: {arr_acc_test}")
    print(f"arr_f1score_test: {arr_f1score_test}")
    print(f"arr_gmean_test: {arr_gmean_test}")
    print(f"arr_precision_test: {arr_precision_test}")
    print(f"arr_recall_test: {arr_recall_test}")

    #Average results
    avg_acc_test = np.mean(arr_acc_test)
    avg_f1score_test = np.mean(arr_f1score_test)
    avg_gmean_test = np.mean(arr_gmean_test)
    avg_precision_test = np.mean(arr_precision_test)
    avg_recall_test = np.mean(arr_recall_test)

    #variance of results
    var_acc_test = np.var(arr_acc_test)
    var_f1score_test = np.var(arr_f1score_test)
    var_gmean_test = np.var(arr_gmean_test)
    var_precision_test = np.var(arr_precision_test)
    var_recall_test = np.var(arr_recall_test)

    # print average and variance
    print(f"avg_acc_test: {avg_acc_test:.4f} +/- {np.sqrt(var_acc_test):.4f}")
    print(f"avg_f1score_test: {avg_f1score_test:.4f} +/- {np.sqrt(var_f1score_test):.4f}")
    print(f"avg_gmean_test: {avg_gmean_test:.4f} +/- {np.sqrt(var_gmean_test):.4f}")
    print(f"avg_precision_test: {avg_precision_test:.4f} +/- {np.sqrt(var_precision_test):.4f}")
    print(f"avg_recall_test: {avg_recall_test:.4f} +/- {np.sqrt(var_recall_test):.4f}")
